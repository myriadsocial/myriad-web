...
export class PolkadotJs implements IProvider {
  ...
  async signer(
    ...args: InjectedAccountWithMeta[]
  ): Promise<InjectedAccountWithMeta> {
    const [account] = args;
    const keyring = new Keyring();
    const baseAddress = account
      ? account.address
      : keyring.encodeAddress(this.accountId);
    ...
    if (!currentAccount) {
      throw new NoAccountException(
        'Account not registered on Polkadot.js extension',
      );
    }
  ...
  async balances(
    ...
    if (referenceId) {
      ...
      this.listenToAssetBalanceChange(
        this.accountId,
        referenceId,
        new BN(balance),
        callback,
  ...
  async claimTip(
    ...
    try {
      ...
      const api = this.provider;
      const injector = await web3FromSource(signer.meta.source);
      const extrinsic = api.tx.tipping.claimTip(
        serverId,
        'user',
        referenceId,
        ftIdentifiers,
  ...
  async payTransactionFee(
    ...
    return this.signTippingTransaction(
      walletDetail,
      new BN(trxFee),
      'native',
      account,
  ...
  async estimateFee(
    ...
    try {
      ...
      const extrinsic = api.tx[extType][
        method
      ] as SubmittableExtrinsicFunction<'promise'>;
      const { partialFee } = await extrinsic(dest, RAND_AMOUNT).paymentInfo(
        this.accountId,
      );
  ...
  async assetMinBalance(
    ...
    return {
      partialFee: new BN(assetDetails.minBalance.toString().replace(/,/gi, '')),
    };
  }

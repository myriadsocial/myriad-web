...
export const CommentDetail = forwardRef<HTMLDivElement, CommentDetailProps>(
  (props, ref) => {
    const {
      section,
      comment,
    ...
    const [exclusiveContent, setExclusiveContent] =
      React.useState<ExclusiveContent>();
    ...
    return (
      <>
        <div
          ...
          ref={ref}
          id={`comment-${comment.id}-deep-${deep}`}>
          <div className={style.tree}>
            ...
            />
            {(deep === 0 || deep > 2 || replies.length > 0) && (
              <div className={style.verticalTree} />
            )}

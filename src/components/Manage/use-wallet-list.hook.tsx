  ...
  const walletList = React.useMemo<WalletOption[]>(() => {
    const findWalletId = (platform: BlockchainPlatform) => {
      ...
      wallets.forEach(wallet => {
        if (wallet?.network?.blockchainPlatform === platform)
          walletId = wallet.id;
      });
    ...
    return [
      {
        ...
        isConnect: Boolean(
          wallets.find(
            i => i?.network?.blockchainPlatform === BlockchainPlatform.NEAR,
          ),
        ),
      ...
      },
      {
        ...
        isConnect: Boolean(
          wallets.find(
            i =>
              i?.network?.blockchainPlatform === BlockchainPlatform.SUBSTRATE,
          ),
        ),

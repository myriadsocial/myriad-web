...
export const FriendListContainer: React.FC<FriendListContainerProps> =
  props => {
    const {
      user,
      type,
    ...
    const { detail } = useSelector<RootState, ProfileState>(
      state => state.profileState,
    );
    const { settings } = useSelector<RootState, ConfigState>(
      state => state.configState,
    );
    const userLogin = useSelector<RootState, User | undefined>(
      state => state.userState.user,
    );
    const userFriends = useSelector<RootState, Friend[]>(
      state => state.friendState.friends,
    );
    const profileFriends = useSelector<RootState, Friend[]>(
      state => state.profileState.friends.data,
    );

...
export const FriendRequestListContainer: React.FC<FriendRequestContainerProps> =
  props => {
    const { user } = props;

    const { loadRequests, toggleRequest, removeFriendRequest } =
      useFriendsHook(user);
    const enqueueSnackbar = useEnqueueSnackbar();

    const { requests } = useSelector<RootState, FriendRequestState>(
      state => state.friendRequestState,
    );

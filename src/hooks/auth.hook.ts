...
import {Polkadot} from 'src/lib/services/polkadot-js';
...
export const useAuthHook = () => {
  ...
  const signInWithExternalAuth = async (
    ...
    if (account) {
      const signature = await Polkadot.signWithWallet(account, nonce);
    ...
    try {
      ...
      switch (blockchainPlatform) {
        case BlockchainPlatform.SUBSTRATE:
          ...
          const polkadotSignature = await Polkadot.signWithWallet(polkadotAccount, nonce);
  ...
  const switchNetwork = async (
    ...
    try {
      ...
      switch (blockchainPlatform) {
        case BlockchainPlatform.SUBSTRATE: {
          const polkadotAccount = account as InjectedAccountWithMeta;
          const signature = await Polkadot.signWithWallet(
            polkadotAccount,
            nonce,
            ({signerOpened}) => {
